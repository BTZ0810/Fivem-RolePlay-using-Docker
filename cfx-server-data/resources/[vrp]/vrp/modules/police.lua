...
MySQL.createCommand("vRP/set_gunLicense","UPDATE vrp_users SET GunLicense = @gunlicense WHERE id = @user_id")
...
--PDA
function vRP.setGunLicense(user_id,gunlicense)
  MySQL.execute("vRP/set_gunLicense", {user_id = user_id, gunlicense = gunlicense})
end

function vRP.gunLicense(source)
  local user_id = vRP.getUserId(source)

  if user_id ~= nil then
    vRP.setGunLicense(user_id,"Required")
  end
end
...
-- Remove PDA (UserID)
local choice_gunlicense = {function(player, choice)
  local user_id = vRP.getUserId(player)
   if user_id ~= nil then
    vRP.prompt(player,"UserID: ","",function(player,id)
      id = parseInt(id)
        local source = vRP.getUserSource(id)
        if source ~= nil then
          vRP.gunLicense(source)
              vRPclient.notify(source,"You took the gunLicense from" ..id)
        end
    end)
  end
end, lang.police.menu.gunlicense.description()}
-- Remove PDA (UserID)
    ...
    if vRP.hasPermission(user_id,"police.menu") then
      ...
      choices[lang.police.title()] = {function(player,choice)
        vRP.buildMenu("police", {player = player}, function(menu)
          ...
          if vRP.hasPermission(user_id,"police.jail") then
            menu[lang.police.menu.gunlicense.title()] = choice_gunlicense
          end
