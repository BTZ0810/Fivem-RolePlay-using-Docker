    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
          TriggerClientEvent("pNotify:SendNotification",player,{text = "Destruiu <span color='green'>"..amount.."</span> "..vRP.getItemName(idname), type = "info", timeout = (3000),layout = "centerLeft"})
...
function vRP.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    if notify then
      ...
      if player ~= nil then
        TriggerClientEvent("pNotify:SendNotification",player,{text = "Pegou <span color='green'>"..amount.."</span> "..vRP.getItemName(idname), type = "info", timeout = (3000),layout = "centerLeft"})
      end
    end
  end
end
  ...
  if data and amount > 0 then
    ...
    if entry and entry.amount >= amount then -- add to entry
      ...
      if notify then
        local player = vRP.getUserSource(user_id)
        if player ~= nil then
          TriggerClientEvent("pNotify:SendNotification",player,{text = "Deu <span color='green'>"..amount.."</span> "..vRP.getItemName(idname), type = "info", timeout = (3000),layout = "centerLeft"})
        end
      end
