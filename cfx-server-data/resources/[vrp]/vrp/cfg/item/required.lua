...
items["money"] = {"Dinheiro","Dinheiro empacotado.",function(args)
  ...
  choices["Desempacotar"] = {function(player,choice,mod)
    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player, "Quanto deseja desempacotar? (máximo "..amount..")", "", function(player,ramount)
...
-- money binder
items["money_binder"] = {"Pack de R$ 1.000","Usado para empacotar R$ 1.000 .",function(args)
  local choices = {}
  local idname = args[1]

  choices["Pacote de dinheiro"] = {function(player,choice,mod) -- bind the money
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
...
local wammo_choices = function(args)
  ...
  choices["Recarregar"] = {function(player,choice)
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
