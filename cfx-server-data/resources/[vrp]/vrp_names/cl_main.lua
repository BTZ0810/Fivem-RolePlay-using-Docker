...
function vRPN.insertUser(user_id,source,name,permission,vp)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k,v in pairs(players) do
			...
			--TriggerEvent("mt:missiontext",ped.." - "..ply,0)
			if ((ped ~= ply) or (ped == ply and config.self)) then
				...
				if ((distance < config.range)) and (not config.admin_only or permissions[k]) then
                    ...
                    if config.name then text = text .. names[k] end
