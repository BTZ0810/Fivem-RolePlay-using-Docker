	...
	if distance < 5 then
		if amount >= 1 then
			if status < 100 then
				...
				vRP.notify("Você deu comida ao seu pet.")
				...
				vRP.notify("O seu pet está sem fome.")
			end
		else
			vRP.notify("~sr~Você não tem comida para seu pet, compre na loja~s~")
		end
	else
		vRP.notify("~r~O seu pet está longe!~s~")
	end
end)
...
AddEventHandler("vrp_pets:attachdettach", function ()
	if not IsPedSittingInAnyVehicle(ped) then
		...
		vRP.notify({"~r~Você não pode por seu animal de estimação neste veículo!~s~"})
	end
end)
	...
	if not isInVehicle then
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if distance < 8 then
				...
				vRP.notify({"~r~O seu pet está longe do veículo!~s~"})
			end
		else
			vRP.notify({"É necessario estar dentro do veículo!"})
		end
	else
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			...
			vRP.notify({"Ainda está dentro de um veículo."})
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			...
			DeleteEntity(ped)
			vRP.notify({"O teu animal de estimação está morto! Da proxima vez lembre-se de dar comida"})
			come = 3
			status = "die"
		end
	end
...
Citizen.CreateThread(function()
	...
	AddTextComponentString("~p~Loja de Animais")
