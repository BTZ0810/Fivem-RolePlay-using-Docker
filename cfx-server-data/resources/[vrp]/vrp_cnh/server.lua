...
vRPclient = Tunnel.getInterface("vRP","vrp_cnh")
...
RegisterServerEvent("dmv:buysuccess")
AddEventHandler("dmv:buysuccess", function()
	local user_id = vRP.getUserId({source})
	local player = vRP.getUserSource({user_id})
	if vRP.tryPayment({user_id,5000}) then
        TriggerClientEvent('dmv:EndBuyLicense',player)
	else
