  ...
  while true do
	...
	if IsPedInAnyVehicle(GetPlayerPed(-1), false) and not engine then
	  ...
	  if damage <= 0 then
		...
		vRP.notify({"~r~O motor do carro fundiu."})
	  end
	end
  end
end)
	      ...
	      if skipper or caller then
		    HKserver.docsOnline({},function(docs)
			  ...
			  elseif docs > 0 and caller then
			    vRP.notify({"~g~Há paramédicos de plantão.\n~b~Pressione ~g~K~b~ vá até o celular e chame o SAMU."})
			  end
		    end)
          end
	    end)
...
function vRPhk.lockVehicle(lockStatus, vehicle)
	if lockStatus == 1 or lockStatus == 0 then -- locked or unlocked
		...
		-- ## Notifications
			...
			vRP.notifyPicture({"CHAR_LIFEINVADER", 3, "Positron", "Central de Monitoramento", "O seu veículo está trancado."})
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- if it is locked
		...
		-- ## Notifications
			...
			vRP.notifyPicture({"CHAR_LIFEINVADER", 3, "Positron", "Central de Monitoramento", "O seu veículo está destrancado."})
		-- ## Notifications
