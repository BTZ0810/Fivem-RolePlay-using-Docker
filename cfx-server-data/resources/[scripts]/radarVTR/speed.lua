...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~Iniciando Radar...~w~321...~y~! ",
	info2 = "~y~Iniciando Radar...~w~321...~y~! ",
	minSpeed = 5.0,
	maxSpeed = 75.0,
}
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Iniciando Radar...~w~321...~y~! ")
				radar.info2 = string.format("~y~Iniciando Radar...~w~321...~y~! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
