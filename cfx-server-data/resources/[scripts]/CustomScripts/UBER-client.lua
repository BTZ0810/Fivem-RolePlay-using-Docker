...
function StartJob(jobid)
	if jobid == 1 then -- UBER
		showLoadingPromt("Loading work: UBER Driver", 2000, 3)
	...
	while true do
		...
		if onJob == 0 then
			if IsControlJustPressed(1, 214) or IsDisabledControlJustPressed(1, 214) then -- DEL
				if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
					if IsVehicleModel(GetVehiclePedIsUsing(GetPlayerPed(-1)), GetHashKey("UBER", _r)) then
						StartJob(1)
					end
				end
			end
		elseif onJob == 1 then
			if DoesEntityExist(jobs.cars[1]) and IsVehicleDriveable(jobs.cars[1], 0) then
				...
				StopJob(1)
				DrawMissionText("UBER is ~h~~r~destroyed~w~.", 5000)
			end
		end
	end
end)