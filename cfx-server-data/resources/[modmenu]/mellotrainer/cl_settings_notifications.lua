...
local selfDeathMessage = "~o~Você ~s~morreu."
local deathSuicideMessage = "~o~Você ~s~cometeu suicidio."
...
AddEventHandler( 'mellotrainer:playerJoined', function( ID )
	...
	if ( featurePlayerNotifications and id ~= PlayerId() ) then
		...
		--drawNotification( "~g~<C>"..name.."</C> ~s~chegou na cidade." )
	end
end )
...
AddEventHandler( 'mellotrainer:playerLeft', function( person )
	if ( featurePlayerNotifications ) then
		--drawNotification( "~r~<C>" .. person.name .. "</C> ~s~foi embora da cidade." )
	end
end )
...
function handlePlayerDeathMessage( pedID, currentPed )
	...
	--local msg = "~o~<C>" .. name .. "</C> ~s~morreu, um minuto de silêncio."
	...
	if ( IsPedAPlayer( entity ) ) then
		...
		if ( kname == name ) then
			--msg = "~o~<C>" .. name .. "</C> ~s~cometeu suicídio, mais uma estrelinha no céu."
